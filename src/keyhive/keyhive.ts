...
async function bootstrapKeyhive(options: {
  ...
  // Built-in sync-server identities. Selection order:
  //   1. explicit `serverContactCardJson` and `serverPeerId` options,
  //   2. the KEYHIVE_SERVER_IDENTITY env var ("keyhive-sync" selects
  //      keyhive.sync.automerge.org; anything else keeps the default),
  //   3. the default, subduction.sync.
  // `process` is guarded for browser/service-worker/WASM contexts.

  // subduction.sync (issuer f709c58c...).
  ...
  // keyhive.sync.automerge.org (issuer d7f41e6f...).
  ...
  const useKeyhiveSyncIdentity =
    (globalThis as any)?.process?.env?.KEYHIVE_SERVER_IDENTITY === "keyhive-sync";

  const serverContactCardJson =
    options.serverContactCardJson ??
    (useKeyhiveSyncIdentity
      ? KEYHIVE_SYNC_CONTACT_CARD
      : DEFAULT_SUBDUCTION_CONTACT_CARD);
  const serverPeerIdHardcoded =
    options.serverPeerId ??
    (useKeyhiveSyncIdentity ? KEYHIVE_SYNC_PEER_ID : DEFAULT_SUBDUCTION_PEER_ID);
  console.warn(
